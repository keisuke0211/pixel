...
public:
	...
	// 引数6  : int DisapText　　　 / １文字目が表示されるまでの時間 (初期値 1)
	// 引数7  : int StandTime　　　 / 待機時間 (初期値 10)
	// 引数8  : int EraseTime　　　 / 消えるまでの時間 (初期値 1) ※ 0 は消えない
	// 引数9  : bool bTextBok　　　 / メッセージボックスの表示・非表示 (初期値 true)
	//--------------------------------------------------
	static CText *CText::Create(Box type, D3DXVECTOR3 pos, D3DXVECTOR2 size,const char *Text, float TextSize = 20 ,int DisapText = 1, int StandTime = 10 , int EraseTime = 1,bool bTextBok = true);
