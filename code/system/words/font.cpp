...
void CFont::Load(void)
{
	...
	while (1)
	{
		...
		if (!strcmp(aDataSearch, "NUM_FONT"))
		{
			fscanf(pFile, "%s", &aDataSearch[0]);
			fscanf(pFile, "%d", &m_nMaxFont);		// 最大数

			//m_Font = new CFont::Info[m_nMaxFont];

		}
		else if (!strcmp(aDataSearch, "FONT"))
		{
			fscanf(pFile, "%s", &s_FileName[nCntFont][0]);	// ファイルパス
			fscanf(pFile, "%s", &s_FontName[nCntFont][0]);	// フォント名
			nCntFont++;
		}
	}
}